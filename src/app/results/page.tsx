...
import { ArrowRight, AlertTriangle, Share2, RotateCcw, ExternalLink } from 'lucide-react'
import { decodeResultsFromUrl, maturityLevels } from '@/lib/assessment-scoring'
import { Button, AnimatedSection } from '@/components/shared'

function ResultsContent() {
  const searchParams = useSearchParams()
  const result = decodeResultsFromUrl(searchParams)
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'My Authorization Maturity Assessment Results',
          text: `I scored Level ${result.level} (${result.levelName}) on the Authorization Maturity Assessment.`,
          url: shareUrl,
        })
      } catch (err) {
    ...
    if (diff > 0) {
      return (
        <p className="text-sm text-teal-600 bg-teal-50 px-4 py-2 rounded-lg">
          Good news: Your assessment suggests you're slightly ahead of where you thought you were.
        </p>
      )
    }
    return (
      <p className="text-sm text-amber-700 bg-amber-50 px-4 py-2 rounded-lg">
        Your assessment suggests you may be at Level {result.level}, while you estimated Level {result.selfAssessedLevel}. 
        This gap is common—it's why we built this tool.
      </p>
    )
  }
          ...
          <AnimatedSection delay={0.1}>
            <div className="card p-8 mb-8">
              <div className="flex flex-col md:flex-row items-center gap-6 mb-6">
                <motion.div 
                  className={`w-24 h-24 rounded-2xl ${levelColor} flex items-center justify-center text-white text-4xl font-bold shadow-lg`}
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                ...
                <div className="text-center md:text-left">
                  ...
                  <p className="text-navy-500 italic">{result.levelTagline}</p>
                </div>
              </div>
          ...
          <AnimatedSection delay={0.2} className="mb-8">
            <div className="card p-6">
              ...
              <div className="relative h-3 bg-gray-200 rounded-full mb-6">
                <div className="absolute top-0 left-0 h-full rounded-full bg-gradient-to-r from-maturity-0 via-maturity-2 to-maturity-4 w-full" />
                {maturityLevels.map((level, idx) => (
                  <div
                    key={idx}
                    className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border-2 border-white ${
                      idx === result.level ? 'ring-4 ring-teal-500/30 scale-150' : ''
                    }`}
                    style={{ 
                      left: `${idx * 25}%`, 
                      transform: `translate(-50%, -50%) ${idx === result.level ? 'scale(1.5)' : ''}`,
                      backgroundColor: ['#ef8354', '#f4a261', '#e9c46a', '#8ab17d', '#2a9d8f'][idx]
                    }}
                  />
                ))}
              </div>
              ...
              <div className="card p-6 border-l-4 border-l-amber-500">
                ...
                <ul className="space-y-2">
                  {result.gaps.map((gap, idx) => (
                    <li key={idx} className="flex items-center gap-2 text-navy-600">
          ...
          <AnimatedSection delay={0.4} className="mb-8">
            <div className="card p-6 bg-teal-50 border-teal-200">
              ...
              <p className="text-navy-600 mb-4">
                Based on your role as a <strong>{result.personaName}</strong>, here's where to focus first.
              </p>
              <Button href={`/persona/${result.persona}`} variant="primary">
          ...
          <AnimatedSection delay={0.6} className="mt-12">
            <div className="text-center p-8 rounded-xl bg-navy-900 text-white">
              ...
              <p className="text-navy-300 mb-6">
                See how PlainID approaches authorization.
              </p>
              <a
